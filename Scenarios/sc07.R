...
## User inputs -----
sc_inputs <- list()
sc_inputs$names <- "sc07" # REMEMBER TO UPDATE for new scenarios

sc_inputs$year_change_starts      <- 2019 # Year the change in salt starts
sc_inputs$year_change_completed   <- 2022 # Year the change in salt is completed (linear diffusion). Change then remains for all consequent years
...
sc_inputs$men_salt_other_change   <- -0.096  # Mean change in other salt for men (g/d)
sc_inputs$women_salt_other_change <- -0.096  # Mean change in other salt for women (g/d)
